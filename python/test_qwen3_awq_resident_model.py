"""Compare fully resident Qwen3-14B-AWQ step with lazy dequantized model."""
...
def main():
 torch.set_num_threads(12);sdk=pathlib.Path(os.environ['VULKAN_SDK']);ext=load(name='freetoken_vulkan_ext',sources=[str(HERE/'ext_module.cpp')],extra_include_paths=[str(sdk/'Include'),str(REPO/'include')],extra_ldflags=[f"/LIBPATH:{sdk/'Lib'}",'vulkan-1.lib'],extra_cflags=['/O2','/D_CRT_SECURE_NO_WARNINGS'],verbose=False);base=ext.resident_bytes_total();model=Qwen3Model.from_pretrained(ext,MODEL);tok=AutoTokenizer.from_pretrained(MODEL);token=tok.encode('Hello')[0];t=time.time();ref=model.forward(torch.tensor([[token]]),only_last_logits=True);lazy=time.time()-t;t=time.time();model.pin_to_vram(False);pin=time.time()-t;ws=ResidentQwen3Workspace(ext,model.config,64);t=time.time();got=resident_qwen3_model_step(model,ws,token,0);step=time.time()-t;d=(got-ref[0]).abs();rt=torch.topk(ref[0,-1],10).indices.tolist();gt=torch.topk(got[0],10).indices.tolist();print('lazy',lazy,'pin',pin,'step',step,'resident',ext.resident_bytes_total(),'max',d.max().item(),'mean',d.mean().item(),'top_equal',rt==gt,'top',gt);assert rt==gt;ws.free();model.close();assert ext.resident_bytes_total()==base;print('QWEN3_AWQ_RESIDENT_MODEL_OK')
