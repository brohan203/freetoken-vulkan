...
t=time.time();model=GptOssModel.from_pretrained(ext,MODEL,stream_experts=True);model.enable_streamed_vram_cache(18,'lfu');model.pin_lm_head_to_vram();model.pin_projections_to_vram(False);print('setup_s',time.time()-t,'resident_gib',ext.resident_bytes_total()/1024**3);tok=AutoTokenizer.from_pretrained(MODEL);t=time.time();text,ids,stats=greedy_generate_resident(model,tok,PROMPT,N,256,False);total=time.time()-t;print('text',repr(text));print('tokens',ids);print('prefill',stats['prefill_time'],'decode_avg',sum(stats['decode_times'])/max(1,len(stats['decode_times'])),'total',total,'resident_gib',ext.resident_bytes_total()/1024**3)
