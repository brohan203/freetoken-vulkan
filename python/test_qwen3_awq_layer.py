"""Validate one dequantized Qwen3-14B-AWQ layer."""
...
def main():
 torch.set_num_threads(12);sdk=pathlib.Path(os.environ['VULKAN_SDK']);ext=load(name='freetoken_vulkan_ext',sources=[str(HERE/'ext_module.cpp')],extra_include_paths=[str(sdk/'Include'),str(REPO/'include')],extra_ldflags=[f"/LIBPATH:{sdk/'Lib'}",'vulkan-1.lib'],extra_cflags=['/O2','/D_CRT_SECURE_NO_WARNINGS'],verbose=False);cfg=load_qwen3_config(MODEL);w=load_qwen3_layer(ShardedSafetensors(MODEL),0);torch.manual_seed(171);x=torch.randn(1,3,cfg.hidden_size)*.02;cos,sin=compute_rope(cfg,torch.arange(3));ref=reference_layer(x,w,cfg,cos,sin);got=qwen3_layer_forward(ext,x,w,cfg,cos,sin);d=(got-ref).abs();print('shape',tuple(got.shape),'max',d.max().item(),'mean',d.mean().item(),'finite',torch.isfinite(got).all().item());assert torch.allclose(got,ref,rtol=1e-4,atol=5e-4);print('QWEN3_AWQ_LAYER_OK')
