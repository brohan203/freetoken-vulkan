...
def resident_decode_layer(
    ...
    if resident_moe is not None:
        ext.decode_pre_moe_all_resident(
            x.handle, projection.input_norm, workspace.normalized.handle,
            projection.q_weight, projection.q_bias, workspace.q.handle,
            projection.k_weight, projection.k_bias, workspace.k.handle,
            projection.v_weight, projection.v_bias, workspace.v.handle,
            ...
            projection.sinks, workspace.attention.handle,
            projection.o_weight, projection.o_bias,
            workspace.projected.handle, workspace.residual.handle,
            projection.post_norm, workspace.post_norm.handle,
            projection.router_weight, projection.router_bias,
            workspace.router_logits.handle, hidden, query_heads,
            kv_heads, head_dim, cfg.num_experts, position,
            workspace.capacity, sliding_window, cfg.rms_norm_eps,
