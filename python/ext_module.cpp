...
void qwen3_decode_layer_resident_vulkan(
 int64_t x,int64_t input_norm,int64_t normalized,int64_t qw,int64_t q,int64_t kw,int64_t k,int64_t vw,int64_t v,int64_t qnorm,int64_t knorm,int64_t cos,int64_t sin,int64_t qrot,int64_t krot,int64_t kcache,int64_t vcache,int64_t sinks,int64_t attn,int64_t ow,int64_t projected,int64_t residual,int64_t postnorm,int64_t post,int64_t gatew,int64_t gate,int64_t upw,int64_t up,int64_t activated,int64_t downw,int64_t mlp,int64_t out,int64_t D,int64_t Dff,int64_t Hq,int64_t Hkv,int64_t Hd,int64_t position,int64_t capacity,double eps,double scale){
 auto&ctx=get_ctx();auto&norm=get_pipeline("rmsnorm_f32.comp.spv",3,8);auto&lin=get_pipeline("linear_bf16_resident_f32.comp.spv",4,16);auto&rope=get_pipeline("rope_partial_f32.comp.spv",4,16);auto&append=get_pipeline("kv_cache_append_f32.comp.spv",4,24);auto&attention=get_pipeline("flash_attention_gpt_oss_kv_cache_f32.comp.spv",5,44);auto&add=get_pipeline("vector_add.comp.spv",3,4);auto&glu=get_pipeline("swiglu_f32.comp.spv",3,4);
 struct NPC{uint32_t H;float eps;};NPC nd{(uint32_t)D,(float)eps},nh{(uint32_t)Hd,(float)eps};struct LPC{uint32_t T,N,K,use_bias;};LPC qpc{1u,(uint32_t)(Hq*Hd),(uint32_t)D,0u},kpc{1u,(uint32_t)(Hkv*Hd),(uint32_t)D,0u},dpc{1u,(uint32_t)Dff,(uint32_t)D,0u},opc{1u,(uint32_t)D,(uint32_t)(Hq*Hd),0u},downpc{1u,(uint32_t)D,(uint32_t)Dff,0u};struct RPC{uint32_t BH,S,D,rotary_dim;}qrp{(uint32_t)Hq,1u,(uint32_t)Hd,(uint32_t)Hd},krp{(uint32_t)Hkv,1u,(uint32_t)Hd,(uint32_t)Hd};struct KPC{uint32_t B,Hkv,Snew,D,start,capacity;}cpc{1u,(uint32_t)Hkv,1u,(uint32_t)Hd,(uint32_t)position,(uint32_t)capacity};struct APC{uint32_t S_q,S_kv,D,H_q,H_kv,H_q_per_kv,past_len,sliding_window,use_sinks,capacity;float scale;}apc{1u,(uint32_t)(position+1),(uint32_t)Hd,(uint32_t)Hq,(uint32_t)Hkv,(uint32_t)(Hq/Hkv),(uint32_t)position,0u,0u,(uint32_t)capacity,(float)scale};struct VPC{uint32_t n;}vd{(uint32_t)D},vf{(uint32_t)Dff};
 auto L=[&](int64_t w,int64_t in,int64_t o){return make_descriptor_set(lin,{resident_buf(w),resident_buf(w),resident_buf(in),resident_buf(o)});};auto N=[&](int64_t a,int64_t w,int64_t o){return make_descriptor_set(norm,{resident_buf(a),resident_buf(w),resident_buf(o)});};auto A=[&](int64_t a,int64_t b,int64_t o){return make_descriptor_set(add,{resident_buf(a),resident_buf(b),resident_buf(o)});};VkDescriptorSet n0=N(x,input_norm,normalized),lq=L(qw,normalized,q),lk=L(kw,normalized,k),lv=L(vw,normalized,v),nq=N(q,qnorm,q),nk=N(k,knorm,k),rq=make_descriptor_set(rope,{resident_buf(q),resident_buf(cos),resident_buf(sin),resident_buf(qrot)}),rk=make_descriptor_set(rope,{resident_buf(k),resident_buf(cos),resident_buf(sin),resident_buf(krot)}),ca=make_descriptor_set(append,{resident_buf(krot),resident_buf(v),resident_buf(kcache),resident_buf(vcache)}),at=make_descriptor_set(attention,{resident_buf(qrot),resident_buf(kcache),resident_buf(vcache),resident_buf(sinks),resident_buf(attn)}),lo=L(ow,attn,projected),a0=A(projected,x,residual),n1=N(residual,postnorm,post),lg=L(gatew,post,gate),lu=L(upw,post,up),sg=make_descriptor_set(glu,{resident_buf(gate),resident_buf(up),resident_buf(activated)}),ld=L(downw,activated,mlp),a1=A(residual,mlp,out);
